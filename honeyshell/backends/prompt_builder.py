...
class PromptBuilder:
    ...
    def _system_prompt(self, now: str | None) -> str:
        ...
        lines += [
            ...
            '  "output"       : ONLY the command\'s raw terminal stdout as a',
            "                   string, exactly as the real program prints it.",
            "                   Do NOT include the shell prompt, do NOT echo the",
            "                   command back, do NOT wrap it in markdown/code",
            "                   fences, do NOT add explanation or commentary.",
            "                   Emit only what the program itself would emit, and",
            "                   no more lines than the real tool would. Empty \"\"",
            "                   if the command prints nothing (e.g. mkdir, cd).",
            ...
            "- Be terse: emit exactly the lines the real program would, no more.",
            "  Never pad output with extra rows, summaries, or trailing notes.",
            "  Verbose or explanatory output is both a performance cost and the",
            "  clearest tell that an AI — not a real shell — is answering.",
