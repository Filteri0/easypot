...
```bash
ollama serve            # 另一終端保持執行（見下方「綁定」注意）
ollama pull qwen2.5:14b # 首次要拉模型
curl http://localhost:11434/api/tags   # 確認 Ollama 活著
```

> **Ollama 綁定**：容器透過 `host.docker.internal`（= docker bridge gateway）
> 連宿主機，而 Ollama 預設只綁 `127.0.0.1`，容器連不到。需讓它聽所有介面：
> systemd 管理時 `sudo systemctl edit ollama` 加
> `Environment="OLLAMA_HOST=0.0.0.0"` 後 `daemon-reload && restart`；
> 手動跑則 `OLLAMA_HOST=0.0.0.0 ollama serve`。用 `ss -tlnp | grep 11434`
> 確認綁的是 `*:11434` 而非 `127.0.0.1:11434`。

> **依賴**：LLM 走 httpx 發請求，image 已在 build 時裝好（`.[transport,llm]`）。
> 若蜜罐 log 出現 `httpx is not installed`，代表用到**舊 image**——
> `docker compose build --no-cache` 重建即可。
